  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 flex flex-col">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900/90 border border-slate-800 backdrop-blur-xl p-4 rounded-2xl mb-6 shadow-xl shrink-0">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-white tracking-tight">Stock & Raw Ingredients</h1>
              <p className="text-xs text-slate-400">Inventory levels, BOM materials & low stock warnings</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3 w-full md:w-auto justify-between md:justify-end">
          ...
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2.5 bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-400 hover:to-indigo-500 text-white font-semibold rounded-xl shadow-lg shadow-purple-500/20 transition cursor-pointer text-xs"
          >
      ...
      {lowStockList.length > 0 && (
        <div className="bg-rose-500/10 border border-rose-500/30 p-4 rounded-2xl mb-6 flex items-start space-x-3 text-rose-300 shrink-0">
          <AlertTriangle className="w-5 h-5 shrink-0 text-rose-400 mt-0.5" />
          <div className="flex-1">
            <h3 className="font-bold text-sm text-rose-200">Attention: Low Stock Threshold Warning ({lowStockList.length} Items)</h3>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {lowStockList.map(item => (
                <span key={item.id} className="px-2.5 py-1 bg-rose-900/40 border border-rose-500/30 rounded-lg text-xs font-semibold text-rose-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 shrink-0">
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <div className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-1">Total Ingredients Tracked</div>
          <div className="text-2xl font-bold text-white">{ingredients.length} <span className="text-xs font-normal text-slate-400">Items</span></div>
        </div>
        <div className="bg-slate-900 border border-emerald-500/20 p-4 rounded-2xl bg-gradient-to-b from-emerald-500/5 to-transparent">
          <div className="text-emerald-400 text-xs font-medium uppercase tracking-wider mb-1">Healthy Stock Items</div>
          <div className="text-2xl font-bold text-emerald-400">{ingredients.length - lowStockList.length}</div>
        </div>
        <div className="bg-slate-900 border border-rose-500/20 p-4 rounded-2xl bg-gradient-to-b from-rose-500/5 to-transparent">
          <div className="text-rose-400 text-xs font-medium uppercase tracking-wider mb-1">Low Stock Alerts</div>
          <div className="text-2xl font-bold text-rose-400">{lowStockList.length}</div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4 shrink-0">
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto overflow-x-auto">
          <span className="text-xs text-slate-400 font-semibold shrink-0">Unit:</span>
          {units.map(u => (
            <button
              key={u}
                ...
                {filteredIngredients.map(ing => {
                  ...
                  return (
                    <tr key={ing.id} className="hover:bg-slate-800/40 transition">
                      <td className="p-4 font-bold text-white text-sm">{ing.name}</td>
                      ...
                      <td className="p-4 text-right">
                        <button
                          ...
                          className="px-3 py-1.5 bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 border border-purple-500/20 font-semibold rounded-xl text-xs transition cursor-pointer"
                        >
                          Adjust Stock
                        </button>
                      </td>
      ...
      {showAdjModal && selectedIngredientForAdj && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-md shadow-2xl relative">
            ...
            <h2 className="text-xl font-bold text-white mb-1">Adjust Stock: {selectedIngredientForAdj.name}</h2>
            ...
            <form onSubmit={handleRecordAdjustment} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase text-slate-300 mb-2">Adjustment Type</label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { id: 'RECEIVED', label: 'Received (+)', color: 'text-emerald-400' },
                    { id: 'WASTAGE', label: 'Wastage (-)', color: 'text-rose-400' },
                    { id: 'CORRECTION', label: 'Correction (=)', color: 'text-sky-400' }
                  ].map(t => (
                    <button
                      type="button"
                      key={t.id}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-purple-500 text-sm"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold uppercase text-slate-300 mb-1">Reason / Notes</label>
                <input
                  type="text"
              ...
              <button
                ...
                className="w-full py-3 bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-400 hover:to-indigo-500 text-white font-semibold rounded-xl shadow-lg shadow-purple-500/25 transition cursor-pointer mt-2"
              >
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-md shadow-2xl relative">
            ...
            <h2 className="text-xl font-bold text-white mb-4">Add New Raw Ingredient</h2>

            <form onSubmit={handleCreateIngredient} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase text-slate-300 mb-1">Ingredient Name</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-purple-500 text-xs"
                  >
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase text-slate-300 mb-1">Initial Stock</label>
                  <input
                    type="number"
                    step="0.1"
                    required
                    ...
                    className="w-full px-3 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-purple-500 text-sm"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold uppercase text-slate-300 mb-1">Reorder Level</label>
                  <input
                    type="number"
                    step="0.1"
                    required
